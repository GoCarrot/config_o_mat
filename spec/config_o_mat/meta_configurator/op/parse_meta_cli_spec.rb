...
require 'config_o_mat/meta_configurator/op/parse_meta_cli'

require 'config_o_mat/meta_configurator/memory'

RSpec.describe ConfigOMat::Op::ParseMetaCli do
  def perform
  ...
  let(:state) { ConfigOMat::MetaConfigurator::Memory.new(argv: argv, env: env) }
  let(:env) do
    {
